...
export function TagInput({ selectedTagIds, selectedTagNames, onChange, onFocus }: Props) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    tagsRow: {
      ...
      minHeight: 44, borderWidth: 1, borderColor: c.border, borderRadius: 10,
      paddingHorizontal: 10, paddingVertical: 6, backgroundColor: c.surface,
    },
    tag: { backgroundColor: c.border, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
    tagText: { fontSize: 13, color: c.muted },
    input: { flex: 1, minWidth: 100, fontSize: 15, color: c.text, paddingVertical: 4 },
    suggestions: {
      backgroundColor: c.surface, borderWidth: 1, borderColor: c.border,
      borderRadius: 8, maxHeight: 140, marginTop: 4,
    },
  ...
  const removeTag = (id: number) => {
    ...
    onChange(selectedTagIds.filter((_, i) => i !== idx), selectedTagNames.filter((_, i) => i !== idx));
  };
    ...
    <View style={styles.container}>
      <View style={styles.tagsRow}>
        {selectedTagNames.map((name, i) => (
          <Pressable key={selectedTagIds[i]} style={styles.tag} onPress={() => removeTag(selectedTagIds[i])}>
